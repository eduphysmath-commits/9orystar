...
# --- 3. ANTI-CHEAT JS (ДЫБЫС ПЕН БҰҒАТТАУ) ---
...
if not st.session_state.submitted:
    with st.form("main_physics_form"):
        ...
        st.info("Условие: h = 20 м, v₀ = 10 м/с, g = 10 м/с²")
        q14a = st.text_input("a) Определите время падения мяча на землю (t):")
        q14b = st.text_input("b) На каком расстоянии (L) от подножия башни упадет мяч?")
...
search_name = st.text_input("Введите имя для поиска:")
if search_name:
